...
app.use(ctx => {
  ...
  const file = ctx.request.files ? ctx.request.files.f2: null
  if (file) {
    ...
    //以 json 形式输出上传文件地址
    ctx.body = `{
      "fileUrl": "${uploadHost}${nextPath.slice(nextPath.lastIndexOf('/'))}"
    }`
  } else {
...
server.listen(port)
console.log(`demo1 server start, port is ${port} ......   `)